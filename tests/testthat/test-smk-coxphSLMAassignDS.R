...
context("coxphSLMAassignDS::smk::simple example, with init")
test_that("simple example, with init", {
    surv_obj <- Surv(time = c(1.0, 1.1, 1.2, 1.3, 1.4, 1.5), event = c(1, 1, 0, 1, 0, 0))
    ...
    weights_arg  <- "weights_obj"
    control_arg  <- survival::coxph.control()
    init_arg     <- c(0.25, 0.75)

    dataObj     <- data.frame(time = c(1.0, 0.0, 0.5, 1.0, 0.9, 0.7, 0.4, 0.3), event = c(TRUE, FALSE, TRUE, FALSE, FALSE, TRUE, TRUE, FALSE), age = c(22, 23, 24, 25, 26, 27, 26, 28), edu = c(100, 105, 110, 90, 95, 100, 100, 105))
    weights_obj <- c(1.0, 0.1, 0.5, 1.0, 0.9, 0.7, 1.0, 0.5)

    res <- coxphSLMAassignDS(formula = formula_arg, dataName = dataName_arg, weights_obj = weights_arg, control = control_arg, init = init_arg)

    expect_length(res, 22)
})

context("coxphSLMAassignDS::smk::simple example, without init with use.rms")
...
context("coxphSLMAassignDS::smk::simple example, with init and use.rms")
test_that("simple example, with init and use.rms", {
    surv_obj <- Surv(time = c(1.0, 1.1, 1.2, 1.3, 1.4, 1.5), event = c(1, 1, 0, 1, 0, 0))
